...
public interface KeywordRepository extends JpaRepository<Keyword, Integer> {
    ...
    @org.springframework.data.jpa.repository.Query(value = "SELECT TOP 50 k.keyword_name, SUM(ISNULL(CAST(p.citation_count + 1 AS FLOAT) / NULLIF(YEAR(GETDATE()) - p.publication_year + 1, 0), 0)) AS TrendScore "
            +
    ...
    @org.springframework.data.jpa.repository.Query(value = "SELECT TOP 50 k.keyword_name, SUM(ISNULL(CAST(p.citation_count + 1 AS FLOAT) / NULLIF(YEAR(GETDATE()) - p.publication_year + 1, 0), 0)) AS TrendScore "
            ...
            "GROUP BY k.keyword_name ORDER BY TrendScore DESC", nativeQuery = true)
    java.util.List<Object[]> findTop50PersonalizedTrendingKeywordNamesWithCount(
            @org.springframework.data.repository.query.Param("fieldId") Integer fieldId);

    @org.springframework.data.jpa.repository.Query(value = "SELECT TOP 10 k.keyword_name, SUM(ISNULL(CAST(p.citation_count + 1 AS FLOAT) / NULLIF(YEAR(GETDATE()) - p.publication_year + 1, 0), 0)) AS TrendScore "
            +
            "FROM keywords k JOIN paper_keywords pk ON k.keyword_id = pk.keyword_id JOIN papers p ON pk.paper_id = p.paper_id "
            +
            "WHERE LOWER(k.keyword_name) LIKE LOWER(:prefix + '%') " +
            "GROUP BY k.keyword_name ORDER BY TrendScore DESC", nativeQuery = true)
    java.util.List<Object[]> findTrendingKeywordsByPrefix(
