...
public interface UserRepository
    ...
    @Query(value = "SELECT COUNT(*) FROM bookmark_papers WHERE user_id = :userId", nativeQuery = true)
    long countBookmarksByUserId(@Param("userId") Long userId);

    @Query(value = "SELECT COUNT(*) FROM follow_topics WHERE user_id = :userId", nativeQuery = true)
    long countFollowedTopicsByUserId(@Param("userId") Long userId);

    @Query(value = "SELECT COUNT(*) FROM follow_authors WHERE user_id = :userId", nativeQuery = true)
    long countFollowedAuthorsByUserId(@Param("userId") Long userId);

    @Query(value = "SELECT COUNT(*) FROM follow_journals WHERE user_id = :userId", nativeQuery = true)
    ...
    @Query(value = "SELECT TOP 3 keyword FROM search_history WHERE user_id = :userId ORDER BY searched_at DESC", nativeQuery = true)
