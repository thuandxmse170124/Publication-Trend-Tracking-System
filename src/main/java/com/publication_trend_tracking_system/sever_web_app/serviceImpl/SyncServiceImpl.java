...
public class SyncServiceImpl implements SyncService {
    ...
    private final ThreadLocal<Map<String, Keyword>> keywordCache = ThreadLocal.withInitial(HashMap::new);
    private final ThreadLocal<Map<String, Topic>> topicCache = ThreadLocal.withInitial(HashMap::new);
    private final ThreadLocal<Map<String, Author>> authorCache = ThreadLocal.withInitial(HashMap::new);
    private final ThreadLocal<Map<String, Journal>> journalCache = ThreadLocal.withInitial(HashMap::new);
    ...
    public SyncJobResponse syncAll(Integer sourceId, Long userId,
            com.publication_trend_tracking_system.sever_web_app.enums.SyncTimeRange timeRange,
    ...
    public void executeSyncJob(Long jobId, Integer sourceId, String customQuery,
        ...
        LocalDate resolvedFromDate = fromDate;
        LocalDate resolvedToDate = toDate;

        if (resolvedFromDate == null && timeRange != null) {
            switch (timeRange) {
                case DAY:
                    resolvedFromDate = LocalDate.now().minusDays(1);
                    break;
                case WEEK:
                    resolvedFromDate = LocalDate.now().minusWeeks(1);
                    break;
                case MONTH:
                    resolvedFromDate = LocalDate.now().minusMonths(1);
                    break;
                case ALL:
        ...
        try {
            ...
            for (String query : queries) {
                SyncJob checkJob = syncJobRepository.findById(jobId).orElse(null);
                if (checkJob != null && "CANCELLED".equalsIgnoreCase(checkJob.getStatus())) {
                    log.info("Sync job {} has been CANCELLED. Aborting...", jobId);
                    return;
                }
                log.info("Starting sync from {} for query: {}", source.getSourceName(), query);
                int page = 1;
                boolean isCustomQuery = (customQuery != null && !customQuery.trim().isEmpty());
                int perPage = isCustomQuery ? 20 : 200;
                int queryProcessedCount = 0;

                while (true) {
                    SyncJob currentJob = syncJobRepository.findById(jobId).orElse(null);
                    if (currentJob != null && "CANCELLED".equalsIgnoreCase(currentJob.getStatus())) {
                        log.info("Sync job {} has been CANCELLED. Aborting...", jobId);
                        return;
                    }
                    String url = buildApiUrl(source, query, page, resolvedFromDate, resolvedToDate, perPage);
                    ...
                    queryProcessedCount += (counts[0] + counts[1]);

                    if (isCustomQuery && queryProcessedCount >= 20) {
                        log.info("Reached 20 papers demo limit for custom query: {}", query);
                        break;
                    }

                    if (!continuePagination) {
                        log.info(
                                "Short-circuiting pagination for query {} as we reached items older than resolvedFromDate.",
                                query);
                        break;
                    }
            ...
            if (!newPapers.isEmpty()) {
                // Collect IDs first to avoid LazyInitializationException on detached entities
                List<Long> newPaperIds = newPapers.stream()
                        .map(p -> p.getPaperId())
                        .filter(id -> id != null)
                        .collect(java.util.stream.Collectors.toList());
                try {
                    log.info("Creating notifications for {} new papers", newPaperIds.size());
                    List<Paper> freshPapers = paperRepository.findAllById(newPaperIds);
                    notificationService.notifyUsersForNewPapers(freshPapers);
                } catch (Exception notifEx) {
                    // Notification failure must NOT affect the sync job status
                    log.error("Failed to send notifications for new papers (job already SUCCESS)", notifEx);
                }
            }
            ...
            job.setStatus("FAILED");
            String rawMsg = ex.getMessage() != null ? ex.getMessage() : "Unknown error";
            job.setErrorMessage(rawMsg.length() > 250 ? rawMsg.substring(0, 247) + "..." : rawMsg);
            job.setFinishedAt(LocalDateTime.now());
            syncJobRepository.save(job);
        }
    }
    ...
    public SyncJobResponse retrySyncJob(Long jobId, Long userId) {
        ...
        return syncFromSource(job.getApiSource().getSourceId(), userId, null,
                com.publication_trend_tracking_system.sever_web_app.enums.SyncTimeRange.ALL, null, null);
    }

    @Override
    @Transactional
    public SyncJobResponse cancelSyncJob(Long jobId) {
        SyncJob job = syncJobRepository.findById(jobId)
                .orElseThrow(() -> new AppException(ErrorCode.SYNC_JOB_NOT_FOUND));
        if ("RUNNING".equalsIgnoreCase(job.getStatus()) || "PROCESSING".equalsIgnoreCase(job.getStatus())) {
            job.setStatus("CANCELLED");
            job.setFinishedAt(LocalDateTime.now());
            job.setErrorMessage("Job cancelled by Administrator.");
    ...
    @Transactional
    public boolean saveResultsInTransaction(String responseBody, ApiSource source, String searchQuery, int[] counts,
            List<Paper> newPapers, LocalDate fromDate, LocalDate toDate) {
        keywordCache.get().clear();
        topicCache.get().clear();
        authorCache.get().clear();
        journalCache.get().clear();
        try {
            ...
            ResearchField researchField = researchFieldRepository.findFirstByFieldNameIgnoreCase(searchQuery)
                    .orElseGet(
                            () -> researchFieldRepository.save(ResearchField.builder().fieldName(searchQuery).build()));

            boolean continuePagination = true;
            if ("OpenAlex".equalsIgnoreCase(source.getSourceName())) {
                continuePagination = parseAndSaveOpenAlex(responseBody, source, topic, searchKeyword, researchField,
                        counts, newPapers, fromDate, toDate);
            } else if ("Semantic Scholar".equalsIgnoreCase(source.getSourceName())) {
                continuePagination = parseAndSaveSemanticScholar(responseBody, source, topic, searchKeyword,
                        researchField, counts, newPapers, fromDate, toDate);
            }
            ...
            String safeMsg = rawMsg.length() > 180 ? rawMsg.substring(0, 177) + "..." : rawMsg;
            throw new RuntimeException("DB transaction error during sync: " + safeMsg, e);
        }
    }

    private String buildApiUrl(ApiSource source, String query, int page, LocalDate fromDate, LocalDate toDate,
            int perPage) throws Exception {
        String encodedQuery = URLEncoder.encode(query, StandardCharsets.UTF_8);
        if ("OpenAlex".equalsIgnoreCase(source.getSourceName())) {
            String url = source.getBaseUrl() + "/works?search=" + encodedQuery + "&per-page=" + perPage + "&page="
                    + page;
            if (fromDate != null && toDate != null) {
                url += "&filter=from_publication_date:" + fromDate.toString() + ",to_publication_date:"
                        + toDate.toString();
            } else if (fromDate != null) {
                url += "&filter=from_publication_date:" + fromDate.toString();
            } else if (toDate != null) {
                url += "&filter=to_publication_date:" + toDate.toString();
            }
            url += "&sort=publication_date:desc";
            return url;
        } else if ("Semantic Scholar".equalsIgnoreCase(source.getSourceName())) {
            int offset = (page - 1) * perPage;
            String url = source.getBaseUrl() + "/v1/paper/search?query=" + encodedQuery + "&limit=" + perPage
                    + "&offset=" + offset
                    + "&fields=title,abstract,authors,journal,year,externalIds,citationCount,fieldsOfStudy";
            if (fromDate != null && toDate != null) {
                url += "&year=" + fromDate.getYear() + "-" + toDate.getYear();
                url += ",publicationDate";
            } else if (fromDate != null) {
                url += "&year=" + fromDate.getYear() + "-";
                url += ",publicationDate";
            } else if (toDate != null) {
                url += "&year=-" + toDate.getYear();
                url += ",publicationDate";
            }
    ...
    private boolean parseAndSaveOpenAlex(
        ...
        if (results.isArray()) {
            for (JsonNode work : results) {
                ...
                if (fromDate != null && pubDate != null && pubDate.isBefore(fromDate)) {
                    return false; // Reached older papers
                }
                if (toDate != null && pubDate != null && pubDate.isAfter(toDate)) {
                ...
                if (dto.title.length() > 250)
                    dto.title = dto.title.substring(0, 247) + "...";
                ...
                dto.paperAbstract = (!abstractNode.isMissingNode() && abstractNode.isObject())
                        ? reconstructAbstractFromJson(abstractNode)
                        : "";
                ...
                if (authorships.isArray()) {
                    for (JsonNode authorship : authorships) {
                        ...
                        if (authorName != null && !authorName.isBlank())
                            dto.authorNames.add(authorName.trim());
                    }
                }
    ...
    private boolean parseAndSaveSemanticScholar(
        ...
        if (data.isArray()) {
            for (JsonNode paperNode : data) {
                ...
                if (fromDate != null) {
                    if (pubDate != null) {
                        if (pubDate.isBefore(fromDate))
                            return false;
                    } else if (year > 0 && year < fromDate.getYear()) {
                        return false;
                    }
                }
                if (toDate != null) {
                    if (pubDate != null) {
                        if (pubDate.isAfter(toDate))
                            continue;
                    } else if (year > 0 && year > toDate.getYear()) {
                        continue;
                    }
                }
    ...
    private void saveOrUpdatePaper(ParsedPaperDTO dto, Topic topic, Keyword searchKeyword, ResearchField researchField,
            ApiSource source, int[] counts, List<Paper> newPapers) {
        Paper paper = null;
        if (dto.doi != null && !dto.doi.isBlank()) {
        ...
        if (isNew) {
            ...
            if (dto.paperAbstract != null && !dto.paperAbstract.isBlank())
                paper.setPaperAbstract(dto.paperAbstract);
            paper.setCitationCount(dto.citations);
            paper.setTitle(dto.title.trim());
            paper.setSourceUrl(dto.sourceUrl);
        ...
        if (dto.journalName != null && !dto.journalName.isBlank()) {
            ...
            String key = jName.toLowerCase();
            Journal journal = journalCache.get().get(key);
            if (journal == null) {
                journal = safeGetOrCreateJournal(jName);
                journalCache.get().put(key, journal);
            }
            paper.setJournal(entityManager.getReference(Journal.class, journal.getJournalId()));
        }

        Set<Author> paperAuthors = new HashSet<>();
        for (String aName : dto.authorNames) {
            String key = aName.trim().toLowerCase();
            Author author = authorCache.get().get(key);
            if (author == null) {
                author = safeGetOrCreateAuthor(aName);
                authorCache.get().put(key, author);
            }
            paperAuthors.add(entityManager.getReference(Author.class, author.getAuthorId()));
        }
        ...
        for (String kwName : dto.keywordNames) {
            String key = kwName.trim().toLowerCase();
            Keyword kw = keywordCache.get().get(key);
            if (kw == null) {
                kw = safeGetOrCreateKeyword(kwName);
                keywordCache.get().put(key, kw);
            }
            keywords.add(entityManager.getReference(Keyword.class, kw.getKeywordId()));
        }
        paper.setKeywords(keywords);

        Set<Topic> topicsSet = new HashSet<>(paper.getTopics() != null ? paper.getTopics() : new HashSet<>());
        if (topic != null)
            topicsSet.add(entityManager.getReference(Topic.class, topic.getTopicId()));
        for (String tName : dto.topicNames) {
            String key = tName.trim().toLowerCase();
            Topic t = topicCache.get().get(key);
            if (t == null) {
                t = safeGetOrCreateTopic(tName);
                topicCache.get().put(key, t);
            }
            topicsSet.add(entityManager.getReference(Topic.class, t.getTopicId()));
        }
    ...
    private Keyword safeGetOrCreateKeyword(String name) {
        for (int attempt = 1; attempt <= 3; attempt++) {
            try {
                return applicationContext.getBean(SyncServiceImpl.class).getOrCreateKeywordInternal(name);
            } catch (Exception e) {
                log.warn("Attempt {} - Duplicate keyword '{}', checking database...", attempt, name);
                Optional<Keyword> kw = keywordRepository.findFirstByKeywordNameIgnoreCase(name);
                if (kw.isPresent()) {
                    return kw.get();
                }
                try {
        ...
        return keywordRepository.findFirstByKeywordNameIgnoreCase(name)
                .orElseThrow(() -> new RuntimeException("Failed to get or create keyword: " + name));
    }

    @Transactional(propagation = org.springframework.transaction.annotation.Propagation.REQUIRES_NEW)
    public Keyword getOrCreateKeywordInternal(String name) {
        return keywordRepository.findFirstByKeywordNameIgnoreCase(name)
                .orElseGet(() -> keywordRepository.saveAndFlush(Keyword.builder().keywordName(name).build()));
    }

    private Topic safeGetOrCreateTopic(String name) {
        for (int attempt = 1; attempt <= 3; attempt++) {
            try {
                return applicationContext.getBean(SyncServiceImpl.class).getOrCreateTopicInternal(name);
            } catch (Exception e) {
                log.warn("Attempt {} - Duplicate topic '{}', checking database...", attempt, name);
                Optional<Topic> t = topicRepository.findFirstByTopicNameIgnoreCase(name);
                if (t.isPresent()) {
                    return t.get();
                }
                try {
        ...
        return topicRepository.findFirstByTopicNameIgnoreCase(name)
                .orElseThrow(() -> new RuntimeException("Failed to get or create topic: " + name));
    }

    @Transactional(propagation = org.springframework.transaction.annotation.Propagation.REQUIRES_NEW)
    public Topic getOrCreateTopicInternal(String name) {
        return topicRepository.findFirstByTopicNameIgnoreCase(name)
                .orElseGet(() -> topicRepository.saveAndFlush(Topic.builder().topicName(name).build()));
    }

    private Author safeGetOrCreateAuthor(String name) {
        for (int attempt = 1; attempt <= 3; attempt++) {
            try {
                return applicationContext.getBean(SyncServiceImpl.class).getOrCreateAuthorInternal(name);
            } catch (Exception e) {
                log.warn("Attempt {} - Duplicate author '{}', checking database...", attempt, name);
                Optional<Author> a = authorRepository.findFirstByFullNameIgnoreCase(name);
                if (a.isPresent()) {
                    return a.get();
                }
                try {
        ...
        return authorRepository.findFirstByFullNameIgnoreCase(name)
                .orElseThrow(() -> new RuntimeException("Failed to get or create author: " + name));
    }

    @Transactional(propagation = org.springframework.transaction.annotation.Propagation.REQUIRES_NEW)
    public Author getOrCreateAuthorInternal(String name) {
        return authorRepository.findFirstByFullNameIgnoreCase(name)
                .orElseGet(() -> authorRepository.saveAndFlush(Author.builder().fullName(name).build()));
    }

    private Journal safeGetOrCreateJournal(String name) {
        for (int attempt = 1; attempt <= 3; attempt++) {
            try {
                return applicationContext.getBean(SyncServiceImpl.class).getOrCreateJournalInternal(name);
            } catch (Exception e) {
                log.warn("Attempt {} - Duplicate journal '{}', checking database...", attempt, name);
                Optional<Journal> j = journalRepository.findFirstByNameIgnoreCase(name);
                if (j.isPresent()) {
                    return j.get();
                }
                try {
        ...
        return journalRepository.findFirstByNameIgnoreCase(name)
                .orElseThrow(() -> new RuntimeException("Failed to get or create journal: " + name));
    }

    @Transactional(propagation = org.springframework.transaction.annotation.Propagation.REQUIRES_NEW)
    public Journal getOrCreateJournalInternal(String name) {
        return journalRepository.findFirstByNameIgnoreCase(name)
                .orElseGet(() -> journalRepository.saveAndFlush(Journal.builder().name(name).build()));
    }
